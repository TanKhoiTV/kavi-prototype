...
| Note | `ort_decoder_jni` is marked **(new)** in §5.1 - it is not yet in the `kavi-android` repo (Build A creates it); the audit row above is target-scale-after-Build-A, not current state. Do not treat as an existing file to extend. |
