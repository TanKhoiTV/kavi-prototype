...
Runtime: PyTorch Transformers (no CTranslate2 support — PR #1808 open since Oct 2024).
"""
...
class MoonshineTinyViCandidate(Candidate):
    ...
    def _infer(self, item: EvalItem) -> tuple[str | None, str | None]:
        ...
        if lang != "vi":
            return None, (
                f"MoonshineTinyViCandidate nhận item không phải vi: "
...
class MoonshineTinyEnCandidate(Candidate):
    ...
    def _infer(self, item: EvalItem) -> tuple[str | None, str | None]:
        ...
        if lang != "en":
            return None, (
                f"MoonshineTinyEnCandidate nhận item không phải en: "
                f"{item.id} (language={lang!r})"
            )
